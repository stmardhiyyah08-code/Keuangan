...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    onLoginOrCreate(email, name);
    setEmail('');
    setName('');
    onClose();
  };

  const isInitialSetup = users.length === 0 || !currentUser;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 my-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <UserCheck className="w-5 h-5 text-blue-600" />
            <h3 className="font-extrabold text-lg text-slate-900 dark:text-white">
              {isInitialSetup ? 'Selamat Datang di Dompetku!' : 'Ganti Profil / Akun Pengguna'}
            </h3>
          </div>
          {!isInitialSetup && (
        ...
        {isInitialSetup && (
          <div className="p-3.5 bg-blue-50 dark:bg-blue-950/40 rounded-2xl border border-blue-200 dark:border-blue-900 text-xs text-blue-800 dark:text-blue-300 font-medium">
            👋 Buat profil akun keuangan Anda terlebih dahulu untuk mulai mengelola dompet & transaksi.
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-3">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">
            {isInitialSetup ? 'Daftar Akun Baru:' : 'Masuk / Tambah Profil Baru:'}
          </span>
